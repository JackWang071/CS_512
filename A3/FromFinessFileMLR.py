...
class FitnessResults:
    ...
    def calc_fitness(self, xi, Y, Yhat, c=2):
        ...
        p = len(xi)
        n = len(Y)
        numer = (((Y - Yhat)**2).sum()/n)**2   # Mean square error
        pcn = p*(c/n)
        if pcn >= 1:
            return 1000
        denom = (1 - pcn)**2
    ...
    def validate_model(self, model, fileW, population, TrainX, TrainY, ValidateX, ValidateY, TestX, TestY):
        ...
        trackDesc, trackFitness,trackModel,trackR2, trackQ2, trackR2PredValidation, trackR2PredTest  = self.InitializeTracks()

        yTrain, yHatTrain, yHatCV, yValidation, yHatValidation, yTest, yHatTest = self.initializeYDimension()
